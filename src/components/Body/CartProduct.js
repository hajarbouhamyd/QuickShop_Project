...
export default class CartProduct extends Component {
    ...
    increaseCounter(item){
        const action = { type: "INCREASE_COUNTER", value: item}
        this.props.dispatch(action)
        this.setState({buttonPressed: true})
    }

    decreaseCounter(item){
        const action = { type: "DECREASE_COUNTER", value: item}
        this.props.dispatch(action)
        this.setState({buttonPressed: true})
    }
                        ...
                        <TouchableOpacity style={{flex:1}}>
                            ...
                            >
                                <View style={{flex:1}}>
                                    ...
                                    <Text style={[styles.textStyle,{fontSize: 13,}]}>Total: ${item.prix * this.props.counter[index]}</Text>
                                </View>

                                <View style={{flex:1,flexDirection:'row', margin: 10,paddingTop: 20}}>
                                    <TouchableOpacity onPress={() => this._delete_Prod(item)}>
                                    ...
                                    <View style={{width: 100,flexDirection:'row', alignItems:'flex-end', justifyContent:'space-around',height:25, marginLeft:80}}>
                                        <Button
                                             onPress={() => this.increaseCounter(item)}
                                            style={styles.buttonStyle}>
                                            <Icon name={'plus'} size={20} style={{color: 'red'}}/>
                                        </Button>
                                        <Text style={{Color: 'gray',fontSize:17}}>{this.props.counter[index]}</Text>
                                        <Button
                                            onPress={() => this.decreaseCounter(item)}
                                            style={styles.buttonStyle}>
                                            <Icon name={'minus'} size={20} style={{color: 'red', }}/>
                                        </Button>
                                    </View>
